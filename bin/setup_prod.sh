...
oc set probe dc/tasks-blue --readiness --get-url=http://:8080/ --initial-delay-seconds=90 --timeout-seconds=2 -n ${GUID}-tasks-prod
oc set probe dc/tasks-blue --liveness --get-url=http://:8080/ --initial-delay-seconds=90 --timeout-seconds=2 -n ${GUID}-tasks-prod
...
oc set probe dc/tasks-green --readiness --get-url=http://:8080/ --initial-delay-seconds=90 --timeout-seconds=2 -n ${GUID}-tasks-prod
oc set probe dc/tasks-green --liveness --get-url=http://:8080/ --initial-delay-seconds=90 --timeout-seconds=2 -n ${GUID}-tasks-prod
# Setting 'wrong' VERSION. This will need to be updated in the pipeline
